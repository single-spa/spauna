...
export async function runCommand(commandName: string, configLocation: string, args: object) {
  // Step 1: Fetch Config
...
async function fetchConfig(configLocation: string) {
  if (configLocation.startsWith("http://") || configLocation.startsWith("https://")) {
